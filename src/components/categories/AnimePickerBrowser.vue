...
const formatSortLabel = (value: string) => value
  .toLowerCase()
  .replaceAll('_', ' ')
...
const emit = defineEmits<{
  selectResult: [result: AniListSearchResult]
  clear: []
}>()
...
const templateSortFieldLabel = computed(() => {
  ...
  return formatSortLabel(templateSort.field)
})
...
const loadResults = async (searchTerm: string, page: number) => {
  ...
  try {
    ...
    const response = await searchAnimeMedia({
      ...
      listVisibility: canUseListFilters.value ? pickerFiltersStore.listVisibility : null,
      search: searchTerm,
      page,
      perPage: pageSize,
      accessToken,
...
watch(() => pickerFiltersStore.listVisibility, (value, previousValue) => {
  if (!props.open || value === previousValue) {
    return
  }
...
<template>
  <div class="min-h-0 flex-1 overflow-y-auto">
    ...
    <PickerResultsFrame
      ...
      @next="currentPage += 1"
      @retry="loadResults(searchDraft, currentPage)"
    >
      <div
